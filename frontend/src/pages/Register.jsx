...
const Register = () => {
  ...
  const sendRequest = () => {
    setError("");
    axios
      .post(`${baseURL}/users/register`, { username, email, password })
      .then((res) => {
        if (res.status === 201) {
          navigate("/login");
        }
      ...
      <div className="w-full max-w-md rounded-2xl bg-white/10 backdrop-blur-2xl border border-white/20 shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-8">
        ...
        <div className="flex flex-col gap-5">
          ...
          <button
            ...
            className="mt-2 py-2.5 rounded-lg bg-amber-400 text-black font-bold tracking-wide hover:bg-amber-300 active:scale-[0.98] transition-all cursor-pointer"
          >
            Create Account
          </button>
